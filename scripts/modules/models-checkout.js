define([
    ...
    function ($, _, Hypr, Backbone, api, CustomerModels, AddressModels, PaymentMethods, HyprLiveContext, blockUiLoader) {
        ...
        function customApiAddPayment(checkoutStep) {
            var order = checkoutStep.getOrder();
            var billingInfo = order.get('billingInfo');
            if (!billingInfo) {
                return checkoutStep.trigger('error', {
                    message: Hypr.getLabel('billingInfoMissingValidError')
                });
            } else {
                return CustomPaymentStrategies.CustomCreditCard(order.apiModel, billingInfo.apiModel.data, billingInfo.data);
            }
        }

        function updateCCData(self, isUpdate, res) {
            ...
            self.fire('sync', JSON.parse(JSON.stringify(self.data)), self.data);
            return self;
        }

        function saveCard(cardData, additionalData) {
            var self = cardData,
                isUpdate = cardData.prop(transform.fields.cardId);
            return cardData.api.action(cardData, (isUpdate ? 'update' : 'save'), makePayload(cardData)).then(function(res) {
                //Modify here if we want to modify CC number into the token or 00000000****
                /*if (additionalData && additionalData.xiToken && additionalData.xiToken != cardData.data.cardNumberPartOrMask) {
                    updateCCData(self, false, res);
                    cardData.data.cardNumberPartOrMask = additionalData.xiToken;
                    cardData.data.cardNumber = additionalData.xiToken;
                    cardData.data.cardNumberPart = additionalData.xiToken;
                    isUpdate = cardData.prop(transform.fields.cardId);
                    return cardData.api.action(cardData, 'update', makePayload(cardData)).then(function(res) {
                        return updateCCData(self, isUpdate, res);
                    });
                } else {*/
                    return updateCCData(self, isUpdate, res);
                //}
        ...
        function makePayload(obj) {
            ...
            if (!data.paymentOrCardType) this.checkoutModel.trigger('error', { message: Hypr.getLabel('cardTypeMissingValidError') });
            if (!data.cardNumberPartOrMask) this.checkoutModel.trigger('error', { message: Hypr.getLabel('cardNumberMissingValidError') });
        ...
        var CustomPaymentStrategies = {
            "CustomCreditCard": function(order, billingInfo, additionalData) {
                var card = order.api.createSync('creditcard', billingInfo.card);
                //errors.passFrom(card, order);
                billingInfo.data = additionalData ? additionalData : billingInfo.data;
                additionalData = additionalData ? additionalData : billingInfo.data;
                return saveCard(card, additionalData).then(function(card) {
             ...
             var self = this, order = this.getOrder();
                ...
                submit: function() {
                    ...
                    if (this.get('paymentType').toLowerCase() === "creditcard") {
                        ...
                        this.callApplyPayment(currentPayment, order, card);
                    }
                },
                doXiInterceptTokenizationCall: function(currentPayment, order, card) {
                    var self = this;
                    ...
                    if (xiToken && card.get('cardNumber') && card.get('cardNumber').indexOf('*') > -1) {
                        ...
                        var myData = $XIPlugin.createJSRequestPacket(HyprLiveContext.locals.themeSettings.xiInterceptMerchantguid, xiToken);
                        myData.addField($XIPlugin.createField('PaymentCreditCard', true, cc));
                        myData.addField($XIPlugin.createField('name', false, name));
                        $XIPlugin.ajax({
                            url: HyprLiveContext.locals.themeSettings.xiInterceptQAEnvironment ? HyprLiveContext.locals.themeSettings.xiInterceptQATokenizeURL : HyprLiveContext.locals.themeSettings.xiInterceptTokenizeURL,
                            type: "POST",
                            data: myData,
                            success: function(a) {
                                self.doXiInterceptResponseCall(currentPayment, order, card, signature, xiToken, cc);
                            },
                            error: function(error) {
                                //The token has expired, request a new one and try again.
                                if(error && !error.HasPassed) {
                                    self.getAccessToken().then(function(res) {
                                        self.doXiInterceptTokenizationCall(currentPayment, order, card);
                                    }, function(error) {
                                        console.log(error);
                                    });
                                } else {
                ...
                doXiInterceptResponseCall: function(currentPayment, order, card, signature, xiToken, cc) {
                    var self = this;
                    var data = {
                        Signature: signature,
                        AccessToken: xiToken
                    ...
                    $.ajax({
                        url: '/xiIntercept/getResponsePacket',
                        type: "POST",
                        data: data,
                        success: function(response) {
                            var xmlDOM = new DOMParser().parseFromString(response.responsePacket, 'text/xml');
                            var jsObject = xmlToJson(xmlDOM);
                            var paymentCreditCard = jsObject.PaymetricResponse ? _.findWhere(jsObject.PaymetricResponse.Fields.FormField, { Name: 'PaymentCreditCard' }) : '';
                            self.get('card').set('cardNumberPartOrMask', cc.replace(/\d(?=\d{4})/g, "0"));
                            if (self.apiModel.data.data) {
                                self.apiModel.data.data.xiToken = paymentCreditCard.Value;
                                self.apiModel.data.data.ccMask = cc.replace(/\d(?=\d{4})/g, "*");
                            } else {
                                self.apiModel.data.data = {
                                    xiToken: paymentCreditCard.Value,
                                    ccMask: cc.replace(/\d(?=\d{4})/g, "*")
                                };
                            }
                            if (self.data) {
                                self.data.xiToken = paymentCreditCard.Value;
                                self.data.ccMask = cc.replace(/\d(?=\d{4})/g, "*");
                            } else {
                                self.data = {
                                    xiToken: paymentCreditCard.Value,
                                    ccMask: cc.replace(/\d(?=\d{4})/g, "*")
                            ...
                            self.callApplyPayment(currentPayment, order, card);
                        },
                ...
                callApplyPayment: function(currentPayment, order, card) {
                    if (!currentPayment) {
                ...
                getAccessToken: function() {
                    ...
                    var deferred = api.defer();
                    $.ajax({
                        url: "/xiIntercept/getAccessToken",
                        success: function(accessToken) {
                            self.parent.set('xiResponsePacket', accessToken);
                            deferred.resolve();
                        },
                        error: function(jqXhr, textStatus, errorThrown) {
                            console.log(errorThrown);
                ...
                getXIToken: function(responsePacket) {
                    if (responsePacket.AccessTokenResponsePacket) {
                        return responsePacket.AccessTokenResponsePacket.ResponsePacket.AccessToken.value;
                    } else {
                        return '';
                    }
                },
                getXISignature: function(responsePacket) {
                    if (responsePacket.AccessTokenResponsePacket) {
                        return responsePacket.AccessTokenResponsePacket.ResponsePacket.Signature.value;
                    } else {
                        return '';
                    }
                },
                applyPayment: function() {
                    ...
                    if (this.nonStoreCreditOrGiftCardTotal() > 0) {
                        if (order.get('billingInfo') && order.get('billingInfo').get('paymentType') === 'CreditCard') {
                            p = customApiAddPayment(this);
                        } else {
                            p = order.apiAddPayment();
                        }
